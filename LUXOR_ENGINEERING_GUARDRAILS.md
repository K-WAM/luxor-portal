...
### C.25 Manual Invite Workflow
- Luxor invites are intentionally manual: admin generates invite, copies link, and sends it outside the system.
...
### C.27 Tenant Dashboard Status Lines
- Tenant dashboard status lines must use existing tenant-scoped APIs. Do not add fake document unread counts when there is no persisted read/view tracking.
...
### C.29 Owner Portal Dashboard vs Reports
- The owner dashboard should stay operational: lead with the Owner Assistant and property/tenant overview, and move investment-analysis-heavy content into the Reports area.
- When owner report sections need the old dashboard analytics, reuse the existing owner investment component rather than rewriting ROI calculations or chart logic.
- Owner payment and maintenance reports must stay read-only and owner-scoped; they may aggregate existing billing and maintenance data, but must not change billing write logic or Stripe routing.

### C.30 Owner Portal Source Files Must Stay UTF-8
- Owner portal page files must be saved as valid UTF-8. Copy-pasted smart punctuation or Windows-1252 characters can pass local editing but fail the Next.js production build with `stream did not contain valid UTF-8`.
- When a build error points to invalid UTF-8, normalize the affected source file back to UTF-8 and replace non-essential special punctuation with plain ASCII where practical.

### C.31 Admin Billing Date Inputs Need Explicit Width
- Browser date inputs in compact admin billing tables can clip their value text if the column is too narrow, even when the overall table has room.
- Fix date visibility by widening the Due column and giving the date input an explicit minimum width. Do not rely on text wrapping for native date inputs.

### C.32 Owner Timeline Labels Need Dedicated Rows
- The Owner Reports unit timeline must reserve distinct vertical rows for lease term, renewal period, and listing period. Overlapping bars make the renewal and listing labels unreadable.
- Timeline month markers should use compact `MMM` plus two-digit year labels so longer lease spans stay scannable without widening the card.

### C.33 Lease Agreements Must Be Additive With Legacy Fallback
- When introducing lease agreements, keep legacy property-level `lease_start`, `lease_end`, and `target_monthly_rent` intact for compatibility and mirror active/upcoming lease values back to those fields until all downstream readers are migrated.
- New lease reads should prefer `lease_agreements` when present, but must fall back cleanly to legacy property fields so the app still works before or during migration rollout.

### C.34 Additive Table Reads Must Survive Missing Migrations
- When a new additive table is introduced, read helpers must tolerate the migration not being applied yet in production. Do not let a missing additive table break legacy pages that should still render from fallback data.
- For Supabase/PostgREST, handle both Postgres missing-table codes and PostgREST table-not-found codes/messages before falling back.

### C.35 Tenant-Sensitive Documents Must Be Lease-Scoped
...
### C.40 Admin Document Quick Actions Must Respect Lease Visibility Constraints
- If a document is tenant-sensitive and still property-wide, do not let quick visibility controls promote it to tenant-visible. The admin must first assign the document to a valid lease through the edit flow.
- When a quick admin action is blocked by a scope rule, surface the specific reason in the UI instead of replacing it with a generic failure message.

### C.41 Document Type Scope Should Default, Not Hard-Lock
- Admin document forms should default scope from document type: lease-related types default to `lease`, all others default to `property`. This is a convenience default, not an uneditable rule.
- Admins may override that default, but tenant-visible lease-related documents still require a valid lease association. Owner-only or admin-only exceptions may remain property-wide when intentionally classified that way.

### C.42 Mixed-Scope Tenant Bills Must Share One Authorization Model
- When `tenant_bills` supports both tenant-specific and lease-level obligations, every tenant-facing read and checkout path must resolve visibility from the same scope rules: tenant rows by `tenant_id`, lease rows by lease membership. Do not let reads, payment authorization, or admin generation drift onto different scope assumptions.
- Lease-bill generation must skip any existing lease-level bill for the same lease/type/due date and also skip conflicting tenant-specific rows for tenants linked to that lease. Shared lease obligations must never be duplicated per co-tenant.

### C.43 Lease Tenant Selection Must Stay Property-Scoped
- Admin lease forms must source tenant options from `user_properties` for the selected property with `role = 'tenant'`. Do not let global user lists or prior lease membership leak cross-property tenants into lease selection.
- Editing a lease may change `lease_agreement_tenants`, but it must not auto-modify `user_properties`. Downstream document visibility, lease-level bills, and reporting should update by reading lease membership directly.

### C.44 Financial Schema Prep Must Be Additive And Dormant
- When preparing Admin Financials for future lease-complete data, recurring expense schedules, or monthly overrides, add schema only. Do not switch any API, calculation, or report to the new fields/tables in the same session unless the user explicitly asks for the logic migration too.
- Existing financial reports remain authoritative on their current columns until a later compatibility-reviewed cutover.

### C.45 Admin Financials Must Separate Inputs Without Breaking Legacy Sources
- Admin Financials may split property-wide fields, lease-linked fields, and recurring expense schedules into separate sections, but property saves must only write the property-owned fields for that section. Do not let a property-wide save silently overwrite lease-linked values.
- Until Monthly Performance automation is explicitly migrated, legacy planned property fields remain the active fallback source and should be labeled as such when recurring schedules are introduced.

### C.46 Admin Monthly Performance Must Compute At Read-Time
- `app/api/admin/financials/monthly/route.ts` is the automation boundary for Admin Monthly Performance. Rent and recurring expenses must be computed at read-time only; no backfills, no silent overwrites of stored `property_monthly_performance` values.
- Source precedence is fixed: `rent_income_override` beats paid rent aggregation, which beats stored `rent_income`; recurring schedules beat stored monthly expense values and only fall back to legacy property fields when no schedule or stored month exists; property tax and market value use overrides before stored values.
- Paid rent must be assigned to the rent month it covers (`year`/`month` or `due_date`), not the date the payment settled. Shared-lease duplicates must be de-duplicated before monthly totals are shown. Future months must not be auto-populated as paid rent unless a paid bill already exists for that billed month.

### C.47 Owner Investment Reports Must Favor Interpretation Over Duplication
- Owner Investment Performance should lead with summary and interpretation, keep detailed metrics secondary, remove non-informative framing cards, and avoid presenting the same expense breakdown in more than one place. Label cleanup in this area must remain display-only unless the user explicitly asks to change calculations.

### C.48 Owner/Admin Financial QA Fixes Must Stay Display-Only
- Owner report status badges must stay contained within their cards at desktop and mobile widths. Prefer responsive flex-wrap layouts and keep the collapsible Detailed Metrics section at the bottom of the report view.
- Admin Financials text cleanups should use ASCII-safe labels (`Show/Hide`, `->`, `>=`, `Yes`) when mojibake appears. Lease-level security deposit entry belongs in the lease workflow and should remain a lease-linked field, not a property-wide financial input.

### C.49 Recurring Expense Frequencies Must Stay End-To-End
- When adding a new recurring-expense frequency, update all three layers together: DB constraint, admin create/edit UI, and any read-time monthly automation that normalizes scheduled values.
- Quarterly recurring schedules are supported and should normalize to a monthly display value using `amount / 3` until a different accounting rule is explicitly requested.

### C.50 Quarterly Schedules Use Real Cadence, Not Amortization
- In Admin Monthly Performance, quarterly recurring schedules must post the full scheduled amount only in the start month and every third month after that (for example Jan/Apr/Jul/Oct from a January start), bounded by the effective date range.
- Do not spread quarterly schedule amounts across every month. If a user wants amortized reporting, that needs to be an explicit separate rule.

### C.51 Paid Rent Aggregation Needs `paid_date` Hygiene
- Admin Monthly Performance rent automation reads paid tenant bills first. Manual status changes to `paid` must stamp `tenant_bills.paid_date`, and read-time aggregation may fall back to `due_date` only for older paid rows that are missing `paid_date`.
- Do not rely on `due_date` when `paid_date` exists. `paid_date` remains the primary source for assigning paid rent into a reporting month.

### C.52 HOA Schedules May Overlap And Must Sum
- A property may have multiple concurrent HOA schedules. Admin validation must allow overlapping HOA date ranges, and Monthly Performance must sum all active HOA schedule rows for a given month.
- Keep overlap blocking for other recurring expense types unless the user explicitly broadens that rule.

### C.53 Stripe Webhooks May Need Dual Secret Verification
- `app/api/stripe/webhook/route.ts` must support separate verified signing secrets for platform and connected-account Stripe events when both flows share one endpoint. Keep legacy `STRIPE_WEBHOOK_SECRET` as a fallback only until rollout is complete.
- Never bypass signature verification, never log secret values, and keep event processing idempotent after verification succeeds.

### C.54 Stripe Payment Status Sync Must Be Transition-Driven
- Stripe payment status automation must update tenant and owner bills only after Stripe webhook events or live Stripe refresh checks confirm a real status transition.
- Use webhook + cron automation for tenant connected-account bills and owner platform invoices, notify `connect@luxordev.com` only when a bill transitions to `processing` or `paid`, and remove manual ACH refresh buttons from admin UI once automated sync exists.
- Do not change Checkout Session creation or payment routing just to compensate for missing status refresh coverage.

### C.55 Owner Reports Mobile Layout Must Stay Contained
- Owner Reports mobile fixes must stay presentation-only: no calculation, API, or billing changes.
- On mobile, the Investment Performance filter area must stack or wrap within the viewport, and Unit Timeline labels must reduce density and use compact date text so they remain readable without horizontal overflow.

### C.56 Owner Bill Visibility Must Follow Property Access
- Owner-facing visibility for `billing_invoices` must be driven by owner property access (`user_properties` / `property_id`), not only by `owner_id` on the invoice row.
- Preserve `owner_id` on `billing_invoices` as a contact, payer, or audit field unless a broader billing-model migration is explicitly approved. Do not change Checkout Session routing just to expand visibility.

### C.57 Admin Dashboard Rent-Paid Badges Must Read Billing Status
- Admin Dashboard property rent-paid badges must use the same paid-rent billing aggregation model as Admin Financials before falling back to stored monthly performance values.
- Count rent as paid for the billed month confirmed by paid `tenant_bills`, de-duplicate shared lease bills, and use stored `property_monthly_performance.rent_income` only as compatibility fallback.

### C.58 Customer Payment Confirmations Must Be Transition-Based
- Payment confirmation emails for tenant bills and owner invoices must be customer-facing, sent to the bill recipient with `connect@luxordev.com` CC'd, and must trigger only on real status transitions.
- ACH sends one email at `processing` and never sends a second email at `paid`; card sends one email at `paid`; manual/Zelle sends one email when manually marked `paid`. Do not alter reminder emails or Stripe Checkout routing to achieve this.

### C.59 Demo Mode Must Stay Admin-Only And Fully Fictional
- Demo Mode must live under `/demo`, be reachable only by admins, and must never appear in owner or tenant portal navigation. Protect it in middleware and again at the route/layout boundary so URL manipulation cannot expose it.
- Demo Mode must read only centralized fictional data, must not call Stripe or Supabase writes, must not send emails, and must reset any simulated payment status changes on refresh.

### C.60 Demo Mode Must Reuse Real Portal UI
- Demo Owner and Tenant views must reuse the current portal layouts, navigation, tabs, cards, tables, and page structure as closely as possible. Use demo adapters/providers to feed fictional data into existing portal pages rather than building parallel demo dashboards.
- Demo-specific visuals inside the portal must stay limited to the shared banner and minimal navigation back to `/demo`; do not show real user emails, real property names, or extra explanatory demo headers inside the reused Owner or Tenant pages.
...
| **Monthly Rent Display (Admin Financials)** | `rent_income_override` -> paid rent from `tenant_bills` grouped by paid month and de-duplicated shared lease month -> stored `property_monthly_performance.rent_income` |
| **Monthly Recurring Expense Display (Admin Financials)** | schedule active for the month -> stored monthly value -> legacy property fallback; future months do not auto-populate schedule amounts |
| **Monthly Total Expenses** | `maintenance + pool + garden + hoa_payments + pm_fee` (property tax excluded) |
| **Monthly Net Income** | `rent_income - total_expenses` |
...
| Version | 5.9 |
| Status | Active |
| Last Updated | 2026-05-03 - v5.9: added C.60 to require Demo Mode to reuse the existing Owner/Tenant portal UI with a demo data adapter instead of parallel demo dashboards, keeping only the shared banner/minimal demo navigation visible. v5.8: added C.59 to require Demo Mode to stay admin-only under `/demo`, use centralized fictional-only data, avoid Stripe/Supabase/email side effects, and reset simulated payment state on refresh. v5.7: added C.58 to require customer-facing payment confirmation emails to be transition-based with `connect@luxordev.com` CC'd, one ACH email at `processing`, one card email at `paid`, one manual/Zelle email at manual `paid`, and no reminder-system changes. v5.6: updated C.46 and C.57 so paid rent display is assigned to the billed month it covers (`year`/`month` or `due_date`), not the settlement date, while still de-duplicating shared lease bills and using stored monthly-performance rent only as fallback. v5.5: added C.57 to require Admin Dashboard rent-paid badges to use the same billing-based paid-rent aggregation as Admin Financials, with shared-lease de-duplication and stored monthly-performance fallback only for compatibility. v5.4: added C.56 to require owner invoice visibility to follow property access rather than only `owner_id`, while preserving `owner_id` as audit/contact metadata and leaving owner Checkout Session routing unchanged. v5.3: added C.55 to require Owner Reports mobile layout fixes to stay presentation-only, keep filters contained on mobile, and reduce Unit Timeline label density with compact date text. v5.2: added C.54 to require transition-driven Stripe status automation via webhook + cron, internal status emails on `processing`/`paid`, and removal of manual ACH refresh UI once automation exists. v5.1: added C.53 to require dual verified Stripe webhook secret support for shared platform + connected-account endpoints, with legacy fallback and no secret leakage in logs. v5.0: added C.52 to allow multiple overlapping HOA schedules and require Monthly Performance to sum all active HOA rows while keeping overlap blocking for other recurring expense types. v4.9: added C.51 to require `paid_date` hygiene for manual paid tenant bills and to allow a read-time `due_date` fallback only for legacy paid rows missing `paid_date`. v4.8: added C.50 to require quarterly recurring schedules to post on their real three-month cadence from the effective start month rather than being amortized across every month. v4.7: added C.49 to require new recurring-expense frequencies to be wired through DB constraints, admin UI, and read-time automation together; quarterly schedules normalize as `amount / 3` until a different rule is requested. v4.6: added C.48 to keep owner-report badge/layout QA fixes and admin-financials ASCII cleanup display-only, and to keep security deposit entry lease-linked in the lease workflow. v4.5: added C.47 to require Owner Investment Performance cleanup to favor summary, interpretation, and one non-duplicated expense view without changing financial calculations. v4.4: added C.46 to require Admin Monthly Performance automation to stay read-time only, honor override/billing/schedule precedence, assign rent by `paid_date`, and de-duplicate shared lease rent before display. v4.3: added C.45 to require Admin Financials sectioned inputs to keep property saves from overwriting lease-linked values and to label legacy planned fields as fallback until automation cutover. v4.2: added C.44 to require additive-but-dormant financial schema prep so lease completeness, recurring expense schedules, and monthly overrides do not silently change active report sources. v4.1: added C.43 to require property-scoped lease tenant selection and to keep lease membership updates separate from `user_properties` while downstream visibility reads lease membership directly. v4.0: added C.42 to require one shared authorization model for mixed tenant/lease bill scope and to prohibit duplicate shared obligations during lease-bill generation. v3.9: added C.41 to require document-type scope defaults without hard-locking admin overrides. v3.8: added C.40 to require admin document quick actions to respect lease visibility constraints and surface specific scope errors. v3.7: added C.39 to require owner-portal AI cards to be removable at the page level without deleting shared AI components or routes. v3.6: added C.38 to require file-preserving admin document metadata edits with strict property/lease validation. v3.5: added C.37 and updated D.7A so owner charts obey the primary Investment Performance filters only and the filter card stays sticky while scrolling. v3.4: added C.36 and D.7A to require one reconciled owner financial-chart dataset with PM fee included in total expenses and non-redundant statement/breakdown/trend outputs. v3.3: added C.35 to require tenant-sensitive documents to be lease-scoped and legacy unscoped rows to stay owner-visible until reassigned. v3.2: added C.34 to require additive table reads to survive missing migrations by falling back cleanly when new tables are absent. v3.1: added C.33 to require additive lease-agreement rollouts with legacy property fallback and mirrored current lease fields for compatibility. v3.0: added C.32 to require dedicated stacked rows and compact month/year markers in the owner unit timeline so renewal/listing labels stay readable. v2.9: added C.31 to require explicit width for compact admin billing date inputs so due dates stay visible in the tenant billing table. v2.8: added C.30 to require UTF-8 owner portal source files after a production build failed on `app/owner/reports/page.tsx` due to invalid encoding. v2.7: added C.29 to lock the owner dashboard as operational-only and move analytics-heavy content into reusable owner reports. v2.6: added C.28 to lock tenant dashboard multi-line upcoming-payment summaries to the viewer's existing access scope without changing billing visibility rules. v2.5: added C.27 to lock tenant dashboard status-line sourcing and priority rules. v2.4: added C.26 to lock invite form minimum fields and single-property owner invite guidance. v2.3: added C.25 to lock the manual invite workflow and canonical invite URL generation. v2.2: plannedYtd made period-aware (YTD/Lease/Alltime); removed actual-rent override from plan; deposit added to plan gross income in lease-start month when last_month_rent_collected; D.8 rewritten; D.0 Plan Gross Income updated; formula accordion updated. v2.1: Added Section 12 "How to Change a Financial Formula" with full checklist, single-source-of-truth rules, past mistakes table, and 6-point verification checklist; v2.0: Full variable consistency audit: D.0 Master Variable Table added (Actual/Plan/YE Target for all 10 variables); deposit model changed to inclusive (last-month deposit counted in gross income for the month received); displayYtd removed -> showDepositBreakdown; calculatedYeTarget dead useMemo removed; YE Target monthly table row now includes PM fee; formula accordion updated to deposit-inclusive model; Projected ROI unified to single calculateExpectedRoi() with PM fee across all three locations (admin dashboard, admin financials, owner page) |
