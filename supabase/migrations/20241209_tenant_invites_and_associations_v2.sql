...
DROP POLICY IF EXISTS "Admins can manage all invites" ON tenant_invites;
DROP POLICY IF EXISTS "Users can view invites for their email" ON tenant_invites;
DROP POLICY IF EXISTS "Admins can manage all user properties" ON user_properties;
...
-- Drop existing function if it exists (to avoid return type mismatch errors)
